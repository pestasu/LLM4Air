...
class GWNET(nn.Module):
    def __init__(self, args, device, dim_in, dim_out):
        ...
        receptive_field = self.output_dim # 1
        ...
        for b in range(self.blocks): # 4
            additional_scope = self.kernel_size - 1 # 2-1
            new_dilation = 1
    ...
    def forward(self, source):
        ...
        inputs = inputs.transpose(1, 3).transpose(3, 2)  # batch, C, N, seq_len
        inputs = nn.functional.pad(inputs, (1, 0, 0, 0))  # batch, C, N, seq_len+1
        ...
        if in_len < self.receptive_field:
            ...
            x = inputs
        x = self.start_conv(x)  # (batch, residual_channels, N, self.receptive_field)
        ...
        for i in range(self.blocks * self.layers):
            ...
            residual = x # (batch_size, residual_channels, num_nodes, self.receptive_field)
            
            # dilated convolution
            filter = self.filter_convs[i](residual) # (batch_size, dilation_channels, num_nodes, receptive_field-kernel_size+1)
            filter = torch.tanh(filter)
            gate = self.gate_convs[i](residual) # (batch_size, dilation_channels, num_nodes, receptive_field-kernel_size+1)
            gate = torch.sigmoid(gate)
            x = filter * gate # (batch_size, dilation_channels, num_nodes, receptive_field-kernel_size+1)
            
            # parametrized skip connection
            s = x # (batch_size, dilation_channels, num_nodes, receptive_field-kernel_size+1)
            s = self.skip_convs[i](s) # (batch_size, skip_channels, num_nodes, receptive_field-kernel_size+1)
            try:
                skip = skip[:, :, :,  -s.size(3):]
            except(Exception):
                skip = 0
            skip = s + skip # (batch_size, skip_channels, num_nodes, receptive_field-kernel_size+1)
