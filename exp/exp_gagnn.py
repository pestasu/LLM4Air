...
class Exp_gagnn(Exp_Basic):
    def __init__(self, args, ii):
        super(Exp_gagnn, self).__init__(args)
        self.cur_exp = ii
        self.rec_mae = nn.L1Loss()

        self.edge_w = np.load(f'{self.data_floder}/local_edge/edge_w.npy',allow_pickle=True)
        self.edge_index = np.load(f'{self.data_floder}/local_edge/edge_index.npy',allow_pickle=True)
        self.loc = np.load(f'{self.data_floder}/local_edge/loc_filled.npy',allow_pickle=True)
        self.edge_index = torch.tensor(self.edge_index)
        self.edge_w = torch.FloatTensor(self.edge_w)
        self.loc = torch.FloatTensor(self.loc)
    ...
    def train_batch(self, x, y):
        ...
        outputs = self.model(x, self.edge_index, self.edge_w, self.loc)
    ...
    def valid(self, epoch):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y) in enumerate(self.dataloader['valid']):
                ...
                outputs = self.model(batch_x, self.edge_index, self.edge_w, self.loc)
